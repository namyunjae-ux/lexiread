...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cell.innerHTML = `
      ...
      ${readCount >= 5 ? '<span class="cal-day-read-count">5/5 ✓</span>' : (readCount > 0 ? `<span class="cal-day-read-count">${readCount}/5</span>` : '')}
    `;

    cell.title = `Click to load columns for ${dateKey} (${readCount}/5 completed)`;

    // Clicking a date cell loads that day's archive!
